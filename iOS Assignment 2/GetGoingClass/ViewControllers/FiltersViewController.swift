...
class FiltersViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let filterValues = FilterService.shared.delegate?.getSetFilterValues()
        radiusSlider.value = Float(filterValues?.0 ?? Constants.defaultRadius)
        rankBySelectedLabel.text = filterValues?.1
        isOpenNow.isOn = Bool(filterValues?.2 ?? Constants.isOpenNow)
    ...
    @IBAction func onSave(_ sender: UIBarButtonItem) {
        print("Filter values saved!")
        FilterService.shared.delegate?.setFilterValues(radius: Int32(radiusSlider.value), rankby: rankBySelectedLabel.text?.lowercased(), isOpen: isOpenNow.isOn)
        dismiss(animated: true, completion: nil)
    }
    
    @IBAction func resetFilterValues(_ sender: Any) {
        radiusSlider.setValue(Float(Constants.defaultRadius), animated: true)
        rankBySelectedLabel.text = rankByDictionary.first?.description()
        isOpenNow.isOn = Constants.isOpenNow
        FilterService.shared.delegate?.setFilterValues(radius: Int32(radiusSlider.value), rankby: rankBySelectedLabel.text?.lowercased(), isOpen: isOpenNow.isOn)
    }
    /*
